...
const Comments = ({ blog }) => {
    ...
    useEffect(() => {
        const getComments = async () => {
            try {
                const res = await axios.get(`http://localhost:5000/comments/${blog._id}`);
                if(res.status === 200) {
    ...
    const AddComment = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.post("http://localhost:5000/comment", comment, { withCredentials: true });
    ...
    const handleDeleteComment = async (id) => {
        try {
            const res = await axios.delete(`http://localhost:5000/comment/${id}`, { withCredentials: true });
